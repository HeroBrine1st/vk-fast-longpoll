...
module.exports = class {
    ...
    update(data, self)
    {
        ...
        switch(data[0])
        {
            ...
            case 5://Message edit
            {
                for(let i in self._callback.edit)
                    process.nextTick(self._callback.edit[i], new Message(data[1], data[2], data[3], data[4], data[5], data[6], (data[7] != undefined) ? (data[7]) : []));
                break;
            }
        }
    }
    ...
    on(event, callback)
    {
        switch(event)
        {
            ...
            case 'edit':
            {
                this._callback.edit.push(callback);
                break;
            }
            case 'incomingread':
            {
